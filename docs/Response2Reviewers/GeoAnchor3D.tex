...
\item[(iv)] \textcolor{blue}{Extensive experiments demonstrate consistent improvements across three evaluated task families, including grounding, question answering, and dense captioning, on four ScanNet-based indoor benchmarks.}

% Extensive experiments on cross-modal grounding, question answering, and dense captioning demonstrate that GeoAnchor3D consistently outperforms strong baselines and generalizes effectively across diverse 3D scene understanding tasks.
...
\textcolor{blue}{GeoAnchor3D follows the object-centric scene representation and preprocessing pipeline of Chat-Scene~\cite{chatscene2024}, including Mask3D~\cite{mask3d2023}-based object proposals, object identifiers, Uni3D~\cite{uni3d2024} and DINOv2~\cite{dinov22023} feature extraction, and the language-model interface. Specifically, IGGA modifies object--object interaction by injecting instruction-conditioned geometric priors into multi-head attention, while GATH regulates the transmission of geometric information across the LLM layers. Thus, the main distinction from Chat-Scene~\cite{chatscene2024} lies in task-adaptive geometric regulation rather than in the upstream proposal generator or the language-model backbone.}
...
As shown in Fig.~\ref{fig:igga}, IGGA consists of four stages. First, object features $\mathbf{F}_{obj}$ are processed through self-attention to capture object-level contextual representations for subsequent geometric modulation. \textcolor{blue}{For each head $h$, the query, key, and value representations are computed using learnable projection matrices $\mathbf{W}_{Q}^{(h)}, \mathbf{W}_{K}^{(h)}, \mathbf{W}_{V}^{(h)} \in \mathbb{R}^{D \times d_k}$, where $d_k=D/H$. Projection biases are omitted from the notation for brevity:}
\textcolor{blue}{
\begin{align}
    \mathbf{Q}^{(h)} &= \mathbf{F}_{\mathrm{obj}}\mathbf{W}_{Q}^{(h)},\\
    \mathbf{K}^{(h)} &= \mathbf{F}_{\mathrm{obj}}\mathbf{W}_{K}^{(h)},\\
    \mathbf{V}^{(h)} &= \mathbf{F}_{\mathrm{obj}}\mathbf{W}_{V}^{(h)}.
\end{align}
}
\begin{equation}
% \mathbf{A}_{sem} = \text{Softmax}\left(\frac{\mathbf{Q}\mathbf{K}^\top}{\sqrt{d_k}}\right),
\textcolor{blue}{
    \mathbf{A}_{\mathrm{sem}}^{(h)}
    =
...
\textcolor{blue}{Here, $H$ is the number of attention heads, $\mathbf{Q}^{(h)}, \mathbf{K}^{(h)}, \mathbf{V}^{(h)} \in \mathbb{R}^{N \times d_k}$, and $\mathbf{A}_{\mathrm{sem}}^{(h)} \in \mathbb{R}^{N \times N}$.}
...
\textcolor{blue}{The resulting five-dimensional descriptor provides a compact and interpretable spatial prior that captures relative distance and direction. Its compatibility with object features is learned through a projection, while its influence on attention is regulated by instruction-conditioned per-head gates.}
...
\textcolor{blue}{where $\mathbf{F}_{obj}^{(j)} \in \mathbb{R}^{D}$ is the feature vector of object $j$, $[\cdot;\cdot]$ denotes concatenation, $\mathbf{W}_p \in \mathbb{R}^{5 \times (D+3)}$ is a learnable weight matrix, and $\sigma$ denotes a sigmoid function. The spatial mask $\mathbf{M}_{\mathrm{spatial}}$ is shared across all attention heads and therefore does not carry a head index.}
...
% \begin{equation}
% \mathbf{M}_{\mathrm{effective}}^{(h)} = \alpha_h \mathbf{M}_{\mathrm{spatial}} + (1-\alpha_h)\mathbf{1},
% \end{equation}
...
% \mathbf{F}_{obj}.
% \end{equation}
\textcolor{blue}{In the fourth stage, the effective spatial mask for head $h$ is computed as:}

\begin{equation}
\textcolor{blue}{\mathbf{M}_{\mathrm{effective}}^{(h)} = \alpha_h \mathbf{M}_{\mathrm{spatial}} + (1-\alpha_h)\mathbf{1},}
\end{equation}

\textcolor{blue}{where $\mathbf{1}\in\mathbb{R}^{N\times N}$ is an all-ones matrix. The fused attention for each head is then obtained by element-wise modulation followed by row-wise normalization:}

\begin{equation}
\textcolor{blue}{\mathbf{A}_{\mathrm{fused}}^{(h)} = \operatorname{Normalize} \left(\mathbf{A}_{\mathrm{sem}}^{(h)} \odot \mathbf{M}_{\mathrm{effective}}^{(h)} \right).}
\end{equation}

\textcolor{blue}{The operator $\operatorname{Normalize}$ rescales each row. For $\widehat{\mathbf{X}} =\operatorname{Normalize}(\mathbf{X})$, we define}

\begin{equation}
\textcolor{blue}{\widehat{X}_{ij} = \frac{X_{ij}} {\sum_{k=1}^{N}X_{ik}+\varepsilon}, \qquad \varepsilon=10^{-8}.}
\end{equation}

\textcolor{blue}{Each head produces $\mathbf{O}^{(h)} =\mathbf{A}_{\mathrm{fused}}^{(h)}\mathbf{V}^{(h)} \in\mathbb{R}^{N\times d_k}$. The head outputs are concatenated along the feature dimension and projected:}
\begin{equation}
\textcolor{blue}{ \mathbf{Z} = \operatorname{Concat} \left(\mathbf{O}^{(1)},\ldots,\mathbf{O}^{(H)} \right)\mathbf{W}_O,}
\end{equation}
\textcolor{blue}{where $\mathbf{W}_O\in\mathbb{R}^{D\times D}$ is a learnable output projection. The refined object features are obtained through dropout, residual addition, and layer normalization:}
\begin{equation}
\textcolor{blue}{\mathbf{F}_{obj}^{*} = \operatorname{LayerNorm} \left(\mathbf{F}_{obj} + \operatorname{Dropout}(\mathbf{Z})\right).}
\end{equation}
...
\paragraph*{Datasets} We evaluate GeoAnchor3D on four datasets spanning three tasks: cross-modal grounding on ScanRefer~\cite{scanrefer2020} (single-object) and Multi3DRefer~\cite{multi3drefer2023} (multi-object), question answering on ScanQA~\cite{scanqa2022}, and dense captioning on Scan2Cap~\cite{scan2cap2021}. All datasets are built upon ScanNet~\cite{scannet2017}. To support instruction-conditioned evaluation across heterogeneous tasks, we convert all benchmarks into a unified instruction-following format~\cite{chatscene2024}.\textcolor{blue}{Although the four benchmarks cover different task formulations and annotation protocols, they share the underlying ScanNet~\cite{scannet2017} indoor-scene distribution. Together, these benchmarks evaluate multi-task performance within the ScanNet~\cite{scannet2017} indoor-scene domain.}

\paragraph*{Evaluation Metrics} Following prior work~\cite{chatscene2024}, we report Acc@$\{0.25, 0.5\}$ for ScanRefer~\cite{scanrefer2020}, F1@$\{0.25, 0.5\}$ for Multi3DRefer~\cite{multi3drefer2023}, CIDEr@0.5 and BLEU-4@0.5 for Scan2Cap~\cite{scan2cap2021}, and CIDEr together with BLEU-4 for ScanQA~\cite{scanqa2022}.
...
\end{equation}
where $\lambda_{\text{gate}}=1.0$ and $\lambda_{\text{coord}}=0.1$. \textcolor{blue}{The LoRA rank, learning-rate schedule, and auxiliary-loss weights follow our established training configuration without a dedicated hyperparameter search.}
...
We conduct ablation studies to isolate the contributions of individual modules and design choices. \textcolor{blue}{The gating-design ablation in Table~\ref{tab:gating_ablation} is trained on ScanRefer~\cite{scanrefer2020} and ScanQA~\cite{scanqa2022}, whereas the gating-granularity and supervision ablation in Table~\ref{tab:gating_granularity_prior} is trained jointly on all four datasets. Comparisons between variants are made within each training setting.}
...
\caption{\textbf{Controlled ablation of gating granularity and gate supervision.} All variants retain GATH and use the same training protocol. The dynamic scalar variant predicts one instruction-conditioned coefficient shared by all heads, while ``w/o gate prior'' removes $\mathcal{L}_{\mathrm{gate}}$.}
\label{tab:gating_granularity_prior}
\resizebox{\textwidth}{!}{
\begin{tabular}{lccccccccccc}
\toprule
\multirow{2}{*}{\textbf{Gating configuration}}
& \multicolumn{2}{c}{\textbf{ScanRefer}}
& \multicolumn{2}{c}{\textbf{Multi3DRefer}}
& \multicolumn{5}{c}{\textbf{ScanQA}}
& \multicolumn{2}{c}{\textbf{Scan2Cap}} \\
...
& Acc@0.25 & Acc@0.5
& F1@0.25 & F1@0.5
& B-1 & B-4 & METEOR & ROUGE-L & CIDEr
& C@0.5 & B-4@0.5 \\
\midrule
Dynamic scalar
& 55.9 & 50.6
& 58.7 & 54.1
& 42.8 & 13.9 & 18.4 & 42.0 & 88.6
& \textbf{77.2} & 35.9 \\

Per-head w/o $\mathcal{L}_{\mathrm{gate}}$
& 56.1 & 50.9
& 59.0 & 54.4
& \textbf{43.2} & \textbf{14.3} & 18.4 & 42.1 & 89.7
& 76.7 & 35.8 \\

Full per-head
& \textbf{56.5} & \textbf{51.1}
& \textbf{59.2} & \textbf{54.6}
& \textbf{43.2} & 14.2 & \textbf{18.5} & \textbf{42.2} & \textbf{89.9}
& \textbf{77.2} & \textbf{36.1} \\
\bottomrule
\end{tabular}
}
\end{table*}
...
\paragraph*{Gating Granularity and Supervision}
\textcolor{blue}{Table~\ref{tab:gating_granularity_prior} further separates the effects of instruction-conditioned learnability, head-specific routing, and task-type gate supervision. Replacing the per-head gates with one dynamic scalar reduces ScanRefer~\cite{scanrefer2020} Acc@0.5 from 51.1\% to 50.6\% and Multi3DRefer~\cite{multi3drefer2023} F1@0.5 from 54.6\% to 54.1\%. The scalar variant also decreases ScanQA~\cite{scanqa2022} CIDEr from 89.9 to 88.6. These results indicate that instruction conditioning alone does not fully replace head-specific modulation, particularly when multiple spatial relations must be represented concurrently.}

\textcolor{blue}{Removing $\mathcal{L}_{\mathrm{gate}}$ produces a smaller but consistent degradation on the grounding metrics, reducing ScanRefer~\cite{scanrefer2020} Acc@0.5 to 50.9\% and Multi3DRefer~\cite{multi3drefer2023} F1@0.5 to 54.4\%. The unsupervised gates remain competitive on ScanQA~\cite{scanqa2022}, including a slightly higher BLEU-4 score of 14.3, while the full model achieves higher METEOR, ROUGE-L, and CIDEr. This pattern suggests that the task-type targets act as soft calibration priors rather than the sole source of routing behavior. They stabilize geometry-intensive tasks, whereas the language modeling objective continues to influence instruction-specific gate predictions.}
...
\caption{\textbf{Within-task gate statistics on ScanRefer.} Instructions are grouped by the number of explicit spatial relations. Each gate value is the per-sample mean over attention heads.}
\label{tab:within_task_gating}
\resizebox{0.49\textwidth}{!}{
\begin{tabular}{ccccc}
\toprule
\textbf{Relations} & \textbf{\#Samples} & \textbf{Mean} &
\textbf{Std.} & \textbf{95\% CI} \\
\midrule
0       & 3477 & 0.8972 & 0.0129 & [0.8968, 0.8976] \\
1       & 4734 & 0.8938 & 0.0132 & [0.8934, 0.8941] \\
$\geq2$ & 1297 & 0.8846 & 0.0150 & [0.8838, 0.8854] \\
\bottomrule
\end{tabular}
}
\end{table}
...
\paragraph*{Within-Task Gate Variation}
\textcolor{blue}{To analyze instruction-dependent gate variation within a task, we group ScanRefer~\cite{scanrefer2020} instructions by the number of explicit spatial relations. As shown in Table~\ref{tab:within_task_gating}, the predicted gates remain high because all samples share the grounding-task prior, but they are not constant within the task. The mean gate decreases from 0.8972 for instructions without an explicit relation to 0.8846 for instructions containing at least two relations, with non-overlapping 95\% confidence intervals.}

\textcolor{blue}{Lower gate values allow more unrestricted semantic attention. This pattern may reflect the need to integrate multiple linguistic constraints in relation-rich instructions. Thus, the task-type supervision determines the coarse operating range, while instruction content produces a finer sample-level adjustment.}
...
\paragraph*{Feature Extraction Depth} Beyond the gating design, the effectiveness of GATH also depends on which LLM layers are selected for representation extraction. Following~\cite{lbt2025}, we partition the 32-layer LLaMA into four depth intervals: early alignment (1--8), shallow fusion (9--16), intermediate fusion (17--24), and deep specialization (25--32). \textcolor{blue}{As shown in Table~\ref{tab:layers_ablation}, performance varies across the selected layer intervals.} \textcolor{blue}{The intermediate interval achieves the highest ScanRefer Acc@0.5 and ScanQA CIDEr, although not the highest BLEU-4. This pattern is consistent with a possible balance between geometric preservation and semantic abstraction~\cite{liu2019linguistic,voita2019bottom}. These results support the selected interval under the evaluated setting.}

% Early layers underperform due to insufficient cross-modal integration, whereas deep layers suffer from language-centric representation compression. The intermediate interval achieves the most favorable trade-off between geometric preservation and semantic abstraction, consistent with prior findings~\cite{liu2019linguistic,voita2019bottom}. In contrast, layers 25--32 produce the weakest grounding performance, suggesting that direct supervision on deep representations becomes less compatible with the language modeling objective.
...
\caption{\textbf{Layer-wise linear probing of scene-centered object coordinates.} Lower RMSE and higher mean $R^2$ indicate stronger linear recoverability of 3D coordinates.}
\label{tab:layerwise_probe}
\resizebox{0.49\textwidth}{!}{
\begin{tabular}{ccccc}
\toprule
\multirow{2}{*}{\textbf{Layer}}
& \multicolumn{2}{c}{\textbf{RMSE $\downarrow$}}
& \multicolumn{2}{c}{\textbf{Mean $R^2$ $\uparrow$}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
& Chat-Scene & GeoAnchor3D & Chat-Scene & GeoAnchor3D \\
\midrule
4  & 1.2997 & \textbf{1.0262} &  0.0510 & \textbf{0.3271} \\
8  & 1.3436 & \textbf{1.0090} & -0.0018 & \textbf{0.3508} \\
12 & 1.3542 & \textbf{1.0358} & -0.0331 & \textbf{0.2965} \\
16 & 1.3335 & \textbf{1.0537} & -0.0135 & \textbf{0.2549} \\
20 & 1.3143 & \textbf{1.0690} &  0.0317 & \textbf{0.2370} \\
24 & 1.3377 & \textbf{1.0846} & -0.0064 & \textbf{0.2034} \\
28 & 1.3507 & \textbf{1.1059} & -0.0313 & \textbf{0.1749} \\
32 & 1.3484 & \textbf{1.1027} & -0.0489 & \textbf{0.1876} \\
\midrule
Average
& 1.3352 & \textbf{1.0609}
& -0.0066 & \textbf{0.2540} \\
...
\paragraph*{Layer-Wise Geometric Recoverability}
\textcolor{blue}{To quantify geometric information retained at different LLM depths, we freeze each model and fit an independent linear probe to predict scene-centered object coordinates from the hidden states of layers $\{4,8,12,16,20,24,28,32\}$. Features are extracted from 100 ScanRefer scenes and divided by scene into 76 training and 24 held-out test scenes. The probes are trained with the same split and optimization settings for both models.}

\textcolor{blue}{As reported in Table~\ref{tab:layerwise_probe}, GeoAnchor3D obtains lower RMSE and higher mean $R^2$ at all eight evaluated depths. Averaged across layers, RMSE decreases from 1.3352 to 1.0609, corresponding to a 20.55\% reduction, while mean $R^2$ increases from $-0.0066$ to 0.2540. At the final layer, GeoAnchor3D retains an $R^2$ of 0.1876, whereas Chat-Scene~\cite{chatscene2024} decreases to $-0.0489$. These results indicate that the GeoAnchor3D representations retain more linearly recoverable coordinate information throughout the evaluated hierarchy, including the deepest layers. These results characterize coordinate recoverability in the complete GeoAnchor3D framework, with IGGA and GATH jointly enabled.}

% \textcolor{blue}{This quantitative probe complements the t-SNE visualization in Fig.~\ref{fig:GATH_tsne}. Since the comparison changes the complete GeoAnchor3D framework rather than GATH alone, the result supports framework-level geometric retention and should not be interpreted as isolating the causal contribution of GATH independently from IGGA.}
...
\caption{\textbf{Efficiency comparison on one NVIDIA RTX 3090.} Both models use batch size 1 and FlashAttention-2. Inference generates 32 tokens with greedy decoding. Training time covers forward and backward computation but excludes data transfer and the optimizer step.}
...
& & Time (ms) $\downarrow$ & Throughput $\uparrow$ & Memory (GiB) $\downarrow$
  & Latency (ms) $\downarrow$ & Throughput $\uparrow$ & Memory (GiB) $\downarrow$ \\
\midrule
Chat-Scene
& 6.8217B & 547.87 & 1.8252 & 14.920
& 2491.18 & 0.4014 & 14.090 \\

GeoAnchor3D
& 6.8364B & 553.81 & 1.8057 & 15.164
& 1993.78 & 0.5016 & 14.145 \\

Relative change
& +0.216\% & +1.08\% & -1.07\% & +1.63\%
& -19.97\% & +24.95\% & +0.39\% \\
\bottomrule
\end{tabular}
}
...
\textcolor{blue}{Table~\ref{tab:efficiency} quantifies the aggregate cost of IGGA, GATH, and the auxiliary objectives. GeoAnchor3D introduces 14.71M parameters, increasing the total parameter count by only 0.216\%. Its measured forward--backward time and peak training memory increase by 1.08\% and 1.63\%, respectively.}

\textcolor{blue}{At inference, GATH and its coordinate regression loss are not executed, while IGGA remains active. The peak allocated memory increases by 0.39\%. Both models are evaluated with the same inputs and inference configuration. The measured implementation does not introduce an inference-latency penalty under the fixed-length generation setting.}
...
\textcolor{blue}{To visualize the geometry of the intermediate representations,} we apply t-SNE to $\mathbf{h}_{\text{geo}}^{(i)}$ for each object $i$, computed from intermediate layers, color-coded by physical X-axis coordinate. As shown in Fig.~\ref{fig:GATH_tsne}, after controlling for semantic category, \textcolor{blue}{the embedding space exhibits clustering associated with physical coordinates:} objects with similar X-coordinates tend to form neighboring clusters along a smooth spatial distribution. \textcolor{blue}{These clustering patterns suggest that the intermediate representations retain spatial information, consistent with the coordinate-recoverability results in Table~\ref{tab:layerwise_probe}.}

\section{Limitations and Future Work}
% The performance of GeoAnchor3D still depends on the quality of Mask3D~\cite{mask3d2023}-generated proposals, which may become a bottleneck in cluttered, heavily occluded, or open-world scenes containing novel object categories. Future work could address this limitation through dynamic proposal generation or open-vocabulary detection. In GATH, the selection of intermediate supervision layers is currently predefined. Developing adaptive layer selection strategies may further improve transferability across different backbones and datasets. In addition, IGGA relies on handcrafted geometric cues such as distance and angular relationships, which may not fully capture complex functional semantics. Incorporating learnable spatial embeddings or structured knowledge representations may provide richer relational reasoning capabilities. Finally, extending instruction-conditioned geometric routing to dynamic environments, such as egocentric video understanding, remains an important direction for future research.

\textcolor{blue}{The performance of GeoAnchor3D still depends on the quality of object proposals generated by Mask3D~\cite{mask3d2023}. Consequently, the downstream reasoning modules cannot recover objects that are entirely missed, severely fragmented, or incorrectly merged by the proposal generator, which may become a bottleneck in cluttered, heavily occluded, or open-world scenes containing novel object categories.} Future work could address this limitation through dynamic proposal generation or open-vocabulary detection. In GATH, the selection of intermediate supervision layers is currently predefined. \textcolor{blue}{Evaluating adaptive layer selection strategies with different language-model backbones may provide a more complete understanding of its transferability. In addition, IGGA uses explicit distance and angular cues, which may not fully capture complex functional relationships. Exploring more expressive learnable spatial embeddings and comparing them with the current encoding under the same gating framework is a promising direction for future work.All evaluated benchmarks are derived from ScanNet~\cite{scannet2017} indoor scenes, and the model is trained on the same task families on which it is evaluated. Therefore, the current results characterize within-domain multi-task adaptation rather than zero-shot generalization to unseen tasks or cross-domain transfer. Evaluation on outdoor environments, dynamic scenes, different 3D backbones, and held-out task families remains an important direction for future work.}

\section{Conclusion}

In this paper, we address two major challenges in spatial-aware MLLMs: the rigid nature of modality interaction and the progressive degradation of geometric features in deep layers. To this end, we propose GeoAnchor3D, a framework grounded in hierarchical spatial regulation for task-adaptive multimodal scene understanding. Unlike existing approaches that perform independent feature modulation at isolated stages, GeoAnchor3D couples instruction-conditioned geometric routing at the input level with deep geometric retention at the representation level, enabling adaptive spatial reasoning across heterogeneous tasks. \textcolor{blue}{Experiments on four ScanNet~\cite{scannet2017}-based benchmarks show that GeoAnchor3D improves the evaluated object grounding, question answering, and dense captioning tasks under a shared object-centric training framework. These results support the effectiveness of task-adaptive geometric regulation across the evaluated tasks within the ScanNet~\cite{scannet2017} indoor-scene domain.}
% Extensive experiments demonstrate that this coupled design consistently outperforms state-of-the-art baselines across both geometry-intensive and semantic-oriented tasks, validating the effectiveness of hierarchical spatial regulation for robust 3D scene understanding.
