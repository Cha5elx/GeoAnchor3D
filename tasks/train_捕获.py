...
def evaluate_all(
    ...
    for val_loader in val_loaders:
        new_val_scores = evaluate(model, model_without_ddp, val_loader, epoch, global_step, device, config)
        val_scores = {**val_scores, **new_val_scores}
        
        # 检查是否已经达到目标捕获数量
        if hasattr(model_without_ddp, 'captured_obj_count') and \
           hasattr(model_without_ddp, 'target_capture_num') and \
           model_without_ddp.captured_obj_count >= model_without_ddp.target_capture_num:
            logger.info(f"[Info] Reached target capture count: {model_without_ddp.captured_obj_count} objects, stopping evaluation")
            break
    ...
    # 保存捕获的深层几何特征用于t-SNE可视化
    if is_main_process() and hasattr(model_without_ddp, 'all_geo_features') and len(model_without_ddp.all_geo_features) > 0:
        final_features = np.concatenate(model_without_ddp.all_geo_features, axis=0)
        final_coords = np.concatenate(model_without_ddp.all_true_coords, axis=0)
        
        feat_path = os.path.join(config.output_dir, 'saved_h_geo.npy')
        coord_path = os.path.join(config.output_dir, 'saved_coords.npy')
        
        np.save(feat_path, final_features)
        np.save(coord_path, final_coords)
        logger.info(f"[Success] 成功保存 {final_features.shape[0]} 个物体的深层特征到 {config.output_dir}")
...
def evaluate(
    ...
    for i, batch in tqdm(enumerate(val_loader)):
        ...
        if hasattr(model_without_ddp, 'captured_obj_count') and \
           hasattr(model_without_ddp, 'target_capture_num') and \
           model_without_ddp.captured_obj_count >= model_without_ddp.target_capture_num:
            logger.info(f"[Info] Stopped evaluation early: captured {model_without_ddp.captured_obj_count} objects")
            break
        ...
        with torch.no_grad():
            # ==========================================
            # [新增] 专门用于提取 t-SNE 特征的逻辑
            # ==========================================
            if hasattr(model_without_ddp, 'extract_deep_features'):
                # 提取特征和真实坐标
                features, coords = model_without_ddp.extract_deep_features(**batch)
                
                if features.shape[0] > 0:
                    # 初始化存储列表和计数器
                    if not hasattr(model_without_ddp, 'all_geo_features'):
                        model_without_ddp.all_geo_features = []
                        model_without_ddp.all_true_coords = []
                        model_without_ddp.captured_obj_count = 0
                        model_without_ddp.target_capture_num = 1500 # 画t-SNE收集1500个物体足够了
                        
                    # 存入列表
                    model_without_ddp.all_geo_features.append(features)
                    model_without_ddp.all_true_coords.append(coords)
                    model_without_ddp.captured_obj_count += features.shape[0]
                
                # 极其关键：提取完特征直接 continue，跳过慢吞吞的文本生成过程！
                continue
            # ==========================================
    ...
    val_scores = {}
    # 如果已经达到目标捕获数量，不计算评估分数
    if hasattr(model_without_ddp, 'captured_obj_count') and \
       hasattr(model_without_ddp, 'target_capture_num') and \
       model_without_ddp.captured_obj_count >= model_without_ddp.target_capture_num:
        logger.info(f"[Info] Skipping evaluation score calculation: captured {model_without_ddp.captured_obj_count} objects")
        return val_scores
