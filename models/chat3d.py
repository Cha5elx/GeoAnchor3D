...
class SpatialRelationAttention(nn.Module):
    ...
    def forward(self, objects, positions, instr_embeds, alpha_ablation_mode=0, task_type=None):
        """
        Args:
            objects: [B, N, D]  物体特征
            positions: [B, N, 3] 物体坐标
            instr_embeds: [B, D_instr] 指令特征
            alpha_ablation_mode: 消融实验模式 (0:原始, 1:固定0.5, 2:任务硬编码)
            task_type: 任务类型 (用于模式2), shape: [B]
        """
        B, N, D = objects.shape
        residual = objects
        ...
        # (B) 计算 Gate 值 (Code 2 逻辑) - 根据消融模式
        if alpha_ablation_mode == 0:
            # 原始模式: 使用门控网络动态计算
            ...
            # 视觉定位任务 (1, 3, 4) -> alpha = 1.0
            # 问答任务 (0) -> alpha = 0.0
            if task_type is None:
                gate_values = torch.full((B, self.num_heads), 0.5, device=objects.device)
            else:
                gate_values = torch.zeros((B, self.num_heads), device=objects.device)
                for i in range(B):
                    if task_type[i] in [1, 3, 4]:  # 视觉定位任务
                        gate_values[i] = 1.0
                    else:  # 问答任务
                        gate_values[i] = 0.0
        else:
...
class Chat3D(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 1: 只回归中心点 (3个值)
        self.coord_regression_mode = 0
        
        # [消融实验] 双重聚合策略模式
        # 0: 原始模式 (水平: Average Pooling, 垂直: 可学习权重)
        # 1: 水平 Max Pooling + 垂直 Mean Pooling
        # 2: 水平 Max Pooling + 垂直 Last Layer
        # 3: 水平 Last Token + 垂直 Mean Pooling
        # 4: 水平 Last Token + 垂直 Last Layer
        self.aggregation_mode = 0
    ...
    def forward_train(self, scene_feat, scene_img_feat, scene_locs, scene_mask, obj_ids, assigned_ids, questions, answers, is_eval=False, description_embeds=None, task_types=None, **kwargs):
        ...
        # 注入空间信息特征
        if self.use_spatial_attention:
            object_embed, gate_values = self.spatial_relation_attention(
                object_embed, 
                scene_locs[:, :, :3], 
                instr_embeds,
                alpha_ablation_mode=self.alpha_ablation_mode,
        ...
        if self.use_geometry_aux:
            ...
            if stride > 0:
                ...
                if all_batch_indices:
                    ...
                    feats_full = torch.stack(geo_features_list, dim=0)  # [8, Total_Objs, stride, D]

                    # [消融实验] 双重聚合策略
                    if self.aggregation_mode == 0:
                        # 模式0: 原始 (水平: Average Pooling, 垂直: 可学习权重)
                        feats_mean = feats_full.mean(dim=2)  # [8, Total_Objs, D]
                        h_geo = (feats_mean * layer_weights.view(-1, 1, 1)).sum(dim=0)  # [Total_Objs, D]
                    elif self.aggregation_mode == 1:
                        # 模式1: 水平 Max Pooling + 垂直 Mean Pooling
                        feats_mean = feats_full.max(dim=2)[0]  # [8, Total_Objs, D]
                        h_geo = feats_mean.mean(dim=0)  # [Total_Objs, D]
                    elif self.aggregation_mode == 2:
                        # 模式2: 水平 Max Pooling + 垂直 Last Layer
                        feats_mean = feats_full.max(dim=2)[0]  # [8, Total_Objs, D]
                        h_geo = feats_mean[-1]  # [Total_Objs, D] - 取第24层
                    elif self.aggregation_mode == 3:
                        # 模式3: 水平 Last Token + 垂直 Mean Pooling
                        feats_mean = feats_full[:, :, -1, :]  # [8, Total_Objs, D] - 取最后一个token
                        h_geo = feats_mean.mean(dim=0)  # [Total_Objs, D]
                    elif self.aggregation_mode == 4:
                        # 模式4: 水平 Last Token + 垂直 Last Layer
                        feats_mean = feats_full[:, :, -1, :]  # [8, Total_Objs, D] - 取最后一个token
                        h_geo = feats_mean[-1]  # [Total_Objs, D] - 取第24层
                    else:
                        # 默认使用原始模式
                    ...
                    # [消融实验] 根据模式选择回归目标
                    if self.coord_regression_mode == 1:
                        # 模式1: 只回归中心点 (前3个值)
                        pred_coords = pred_coords[:, :3]  # [Total_Objs, 3]
                        flat_targets = flat_targets[:, :3]  # [Total_Objs, 3]
