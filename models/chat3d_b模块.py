...
class SpatialRelationAttention(nn.Module):
    ...
    def forward(self, objects, positions, instr_embeds):
        ...
        # 在评估阶段捕获数据
        if self.enable_capture and not self.training:
            self.captured_data['alpha'] = gate_values.detach().clone()
            self.captured_data['A_fused'] = fused_attn.detach().clone()
            self.captured_data['spatial_mask'] = spatial_mask.detach().clone()
            self.captured_data['effective_mask'] = effective_mask.detach().clone()
    ...
    def clear_captured_data(self):
        """清除捕获的数据"""
        for key in self.captured_data:
            self.captured_data[key] = None
...
class Chat3D(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Hook捕获开关（评估阶段捕获alpha和A_fused矩阵）
        self.use_hook_capture = False
        # 存储捕获的深层几何特征和坐标
        self.captured_geo_features = []
        self.captured_geo_coords = []
    ...
    def evaluate(self, scene_feat, scene_img_feat, scene_locs, scene_mask, custom_prompt, obj_ids, assigned_ids, is_eval=True, **kwargs):
        ...
        返回:
            ...
            captured_data: 捕获的hook数据（仅当use_hook_capture=True时）
        """
        # 启用hook捕获（如果配置开启）
        if self.use_hook_capture and self.use_spatial_attention:
            self.spatial_relation_attention.enable_capture_mode()
        ...
        # 打印捕获的数据（如果配置开启）
        if self.use_hook_capture and self.use_spatial_attention:
            captured_data = self.spatial_relation_attention.get_captured_data()
            if captured_data['alpha'] is not None:
                print(f"\n=== Hook Captured Data ===")
                print(f"Alpha shape: {captured_data['alpha'].shape}")
                print(f"Alpha values: {captured_data['alpha']}")
                print(f"A_fused shape: {captured_data['A_fused'].shape}")
                print(f"A_fused mean: {captured_data['A_fused'].mean().item():.6f}")
                print(f"A_fused std: {captured_data['A_fused'].std().item():.6f}")
                print(f"spatial_mask shape: {captured_data['spatial_mask'].shape}")
                print(f"spatial_mask mean: {captured_data['spatial_mask'].mean().item():.6f}")
                print(f"effective_mask shape: {captured_data['effective_mask'].shape}")
                print(f"effective_mask mean: {captured_data['effective_mask'].mean().item():.6f}")
                print(f"========================\n")
            self.spatial_relation_attention.clear_captured_data()
            self.spatial_relation_attention.disable_capture_mode()
        
        return output_texts, gate_values

    @torch.no_grad()
    def extract_deep_features(self, scene_feat, scene_img_feat, scene_locs, scene_mask, custom_prompt, assigned_ids, **kwargs):
        """
        [新增] 专用于 t-SNE 可视化的深层特征提取函数。
        不进行文本生成，只截获并返回第 18-24 层的空间聚合隐状态 (h_geo)。
        """
        object_embed, object_img_embed = self.encode_object_feat(scene_feat, scene_img_feat, scene_locs)
        device = object_embed.device
        batch_size = object_embed.shape[0]

        # 1. 文本指令预处理 (复用 evaluate 的逻辑)
        prompts = []
        for i in range(batch_size):
            tmp_prompt = f" {custom_prompt[i]} {self.role[1]}: "
            tmp_prompt = update_caption(tmp_prompt, assigned_ids[i])
        ...
        seq_embeds, mask_text_batch = self.get_text_emb(prompts, device=device)
        mask_expanded = mask_text_batch.unsqueeze(-1).float()
        sum_embeddings = torch.sum(seq_embeds * mask_expanded, dim=1)
        sum_mask = torch.clamp(mask_expanded.sum(dim=1), min=1e-9)
        instr_embeds = sum_embeddings / sum_mask

        # 2. 空间关系注意力 (IAGGA)
        if self.use_spatial_attention:
            object_embed, _ = self.spatial_relation_attention(object_embed, scene_locs[:, :, :3], instr_embeds)
        object_embed = torch.nn.functional.normalize(object_embed, dim=-1)

        proj_object_embed = self.object_proj(object_embed)
        proj_object_img_embed = self.object_img_proj(object_img_embed)

        # 3. 准备送入 LLM 的输入 (复用 forward_train 的逻辑以确保 token 对齐)
        input_embed_list, attn_list = [], []
        batch_obj_indices = []
        batch_target_coords = []
        ...
        for i in range(batch_size):
            valid_len = mask_text_batch[i].sum().item()
            prompt_embed = seq_embeds[i, :valid_len]

            # 获取当前样本的 object token embeddings
            object_list_embed = self.get_object_list_embed(
                proj_object_embed[i],
                proj_object_img_embed[i] if self.add_img_token else None,
                None, scene_mask[i], 0, assigned_ids[i] # 这里的 obj_id 传0即可，不影响特征
            )
            
            # 计算 Object Token 在输入序列中的位置
            obj_seq_len = object_list_embed.shape[0]
            num_valid_objs = scene_mask[i].sum().item()
            current_obj_indices = []
            current_target_coords = []
            ...
            if num_valid_objs > 0:
                ...
                valid_ids = torch.where(scene_mask[i])[0]
                valid_assigned_ids = assigned_ids[i][valid_ids]
                target_locs = scene_locs[i][valid_assigned_ids]
            ...
            # 拼装输入 (不包括 Answer)
            wrapped_embed = torch.cat([p_0_embed, object_list_embed, self.p_1_embed.to(device), prompt_embed], dim=0)
            wrapped_attn = torch.ones(wrapped_embed.shape[:-1], dtype=torch.long, device=device)
            
            input_embed_list.append(wrapped_embed)
            attn_list.append(wrapped_attn)
            max_seq_len = max(max_seq_len, wrapped_embed.shape[0])
        ...
        input_embeds = pad_and_trim(input_embed_list, max_seq_len, padding_value=0).to(device)
        attention_mask = pad_and_trim(attn_list, max_seq_len, padding_value=0).to(device)

        # 4. 送入 LLaMA 进行前向传播 (强制输出 hidden_states)
        with self.maybe_autocast():
            outputs = self.llama_model(
                inputs_embeds=input_embeds,
                attention_mask=attention_mask,
                return_dict=True,
                output_hidden_states=True # 极其关键：必须为 True 才能拿到中间层
            )

        # 5. 提取 18-24 层特征并聚合 (完全复用 forward_train 的 GAATH 逻辑)
        extracted_features = []
        extracted_coords = []
        
        if self.use_geometry_aux and num_valid_objs > 0:
            selected_layers = torch.stack(outputs.hidden_states[18:25], dim=0) # [7, B, L, D]        
            layer_weights = torch.softmax(self.geo_layer_weights, dim=0) # [7]
            offset = torch.arange(stride, device=device)            
            ...
            for i in range(batch_size):
                ...
                # 提取并合并多层特征
                feats_list = [selected_layers[l][i, valid_seq_idx] for l in range(7)]
                feats_full = torch.stack(feats_list, dim=0) # [7, N_valid, stride, D]
                feats_mean = feats_full.mean(dim=2)         # [7, N_valid, D]
                h_geo = (feats_mean * layer_weights.view(-1, 1, 1)).sum(dim=0) # [N_valid, D]
                
                # 保存当前 batch 的有效物体特征和对应的真实坐标
                extracted_features.append(h_geo.cpu().numpy())
                extracted_coords.append(batch_target_coords[i][valid_obj_mask].cpu().numpy())

        # 返回的是 Numpy 数组列表
        return np.concatenate(extracted_features) if extracted_features else np.empty((0, self.llama_dim)), \
               np.concatenate(extracted_coords) if extracted_coords else np.empty((0, 6))
